...
def busqueda_en_profundidad_H_delta0(e,o,acciones):
    return bep_H_rec([],[],e,o,acciones)

def bep_H_rec_delta0(plan, visitados, actual, o,acciones):
    diccs = copy.deepcopy(o)
    if actual.satisface_positivas(agrupar_diccionarios(diccs)):
        return plan
    else:
        ...
        ord_aplicables = ordena_heuristica_delta0(aplicables,actual,o)
...
def ordena_heuristica_delta0(aplicables, estado,o):
    dicc = dict()
    for accion in aplicables:
        nuevo_estado = AcciónPlanificación.aplicar(accion,estado)
        distancia = 0
        for obj in o:
            distancia += Heuristicas.delta0(nuevo_estado, obj)
        dicc[accion]=distancia
